...
public class LoginPCProcedure {
	public static void execute(Entity entity, HashMap guistate) {
		...
		if ((guistate.containsKey("text:Username") ? ((EditBox) guistate.get("text:Username")).getValue() : "").equals(entity.getDisplayName().getString())
			...
			entity.getPersistentData().putString("connectedTo", (guistate.containsKey("text:Core") ? ((EditBox) guistate.get("text:Core")).getValue() : ""));
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal(("You are now logged in to " + entity.getPersistentData().getString("connectedTo") + ".")), false);
		} else {
			if (entity instanceof Player _player && !_player.level().isClientSide())
